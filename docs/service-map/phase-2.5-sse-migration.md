...
- even the interval form (1 req / 2 s) still does a full server-side rebuild on
  ...
  one rebuild _per client per tick_.
...
Even with the data fetch fixed, the _graph itself_ visibly jumps and "redraws
...
- **Expand-only layout** — when a new service/edge appears, _add_ it; never
...
## 3. Architectural principles of _building_ the service map
...
- **It is a derived aggregate, not an append-only list.** Logs and metrics are
  ...
  The service map is a _pure function of all traces_:
...
- **The payload is small and bounded.** Even a busy system is a handful of nodes
  ...
  grow; see §6.3.)
- **Layout is _not_ stable today — and that's the §1b bug.** `<ServiceMap>` keys
  its `{#each}` blocks by `serviceName` / `source||target`, so the DOM elements
  are _not_ torn down on update (good — no remount flash). But `layoutGraph`
  ...
  make the _positions themselves_ sticky (§4.2).
- **The displayed map should be cumulative, not a window over live traces.**
  `buildServiceMap` today aggregates only the _currently retained_ traces, so an
...
over SSE** of the _cumulative_ aggregate, gated by a change cursor. Stability
...
service-map aggregate that is updated _incrementally as spans are ingested_ and
...
- In the `activeTab === 'map'` `$effect`, call `serviceMapStore.connectSSE()` and
  ...
  re-entry is cheap). _(Alternative: connect once at mount like Metrics does, if
  we later want the badge live across tabs — note the tradeoff, don't do both.)_
...
node is on screen it never moves; new nodes/edges are _added_; counts update in
...
- On each snapshot, **diff against `placements`**:
  ...
  - **New node** → assign a layer (BFS depth from roots, as today — but a node's
    ...
    next free `slot` at the _bottom_ of that layer. Compute `{x, y}` from
...
A subtle CSS transition on node `transform` can make the _appearance_ of a new
...
- Sticky placements fix the actual visible "flashing": existing nodes stop
  moving, the graph only _expands_, counts update in place.
...
4. **Position stability (no reflow) — the §1b check.** Snapshot each node `<g>`'s
   `transform` (x/y); stream new traces that add a _new_ node/edge; assert every
